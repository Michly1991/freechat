  ...
  listLedger(userId: string, role: BillingRole, range: Range, limit: number) {
    ...
    const rows = db.prepare(`
      ...
      runtime AS (
        ...
        FROM source
        WHERE run_id IS NOT NULL AND entry_type IN ('usage_charge', 'model_usage_charge', 'agent_usage_charge', 'model_income', 'agent_income')
      ...
      standalone AS (
        SELECT
          ...
          created_at
        FROM source
        WHERE run_id IS NULL OR entry_type NOT IN ('usage_charge', 'model_usage_charge', 'agent_usage_charge', 'model_income', 'agent_income')
      ),
      rows AS (
        SELECT * FROM runtime
        UNION ALL
