...
export const APP_ACTIONS: AppActionMeta[] = [
  ...
  { action: 'pdf.read', title: '下载 PDF 后本地读取', category: 'pdf', risk: 'blocked', description: '服务端不解析 PDF；Agent Client 必须先用 file.download 下载到本地，再用本地脚本/工具处理。', args: { ref: 'file:<id> 或路径' }, ui: '文件面板' },
  { action: 'excel.read', title: '下载 Excel 后本地读取', category: 'excel', risk: 'blocked', description: '服务端不解析 Excel；Agent Client 必须先用 file.download 下载到本地，再用本地脚本/工具处理。', args: { ref: 'file:<id> 或路径' }, ui: '文件面板' },
  { action: 'excel.write', title: '本地生成 Excel 后上传', category: 'excel', risk: 'blocked', description: '服务端不生成 Excel；Agent Client 本地生成后使用 file.upload 或 file.write-local 写回房间。', args: { localPath: '本地文件路径', targetPath: '房间输出路径' }, ui: '文件面板' },
  { action: 'word.read', title: '下载 Word 后本地读取', category: 'word', risk: 'blocked', description: '服务端不解析 Word；Agent Client 必须先用 file.download 下载到本地，再用本地脚本/工具处理。', args: { ref: 'file:<id> 或路径' }, ui: '文件面板' },
  { action: 'word.write', title: '本地生成 Word 后上传', category: 'word', risk: 'blocked', description: '服务端不生成 Word；Agent Client 本地生成后使用 file.upload 或 file.write-local 写回房间。', args: { localPath: '本地文件路径', targetPath: '房间输出路径' }, ui: '文件面板' },
  { action: 'ppt.read', title: '下载 PPT 后本地读取', category: 'ppt', risk: 'blocked', description: '服务端不解析 PPT；Agent Client 必须先用 file.download 下载到本地，再用本地脚本/工具处理。', args: { ref: 'file:<id> 或路径' }, ui: '文件面板' },
  { action: 'ppt.write', title: '本地生成 PPT 后上传', category: 'ppt', risk: 'blocked', description: '服务端不生成 PPT；Agent Client 本地生成后使用 file.upload 或 file.write-local 写回房间。', args: { localPath: '本地文件路径', targetPath: '房间输出路径' }, ui: '文件面板' },
  { action: 'image.read', title: '下载图片后本地处理', category: 'image', risk: 'blocked', description: '服务端不做图片理解/OCR；Agent Client 必须先用 file.download 下载到本地，再用本地视觉/OCR工具处理。', args: { ref: 'file:<id> 或路径' }, ui: '文件面板' },
