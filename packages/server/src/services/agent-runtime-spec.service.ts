...
- 服务端会按权限注入公共知识库、当前 Agent 专属知识库、当前房间知识库的相关条目。
- 公共知识适合通用法规/模板/制度；Agent 专属知识适合个人经验/案例/方法论；房间知识适合客户资料/项目事实/已确认结论。
- 不要把房间客户资料写入公共知识库；不要把某个 Agent 的私有经验当成所有 Agent 通用规则。
- 需要沉淀新知识时，优先说明建议写入哪个 scope，并等待 owner/用户确认。
