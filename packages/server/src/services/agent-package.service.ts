...
`, 'utf8')
  ...
  async writeSystemSkill(dirName: string, title: string, description: string, scriptName: string, scriptContent: string) {
    ...
    await mkdir(join(dir, 'res'), { recursive: true })
    await mkdir(join(dir, 'scripts'), { recursive: true })
    await writeFile(join(dir, 'SKILL.md'), `# ${title}\n\n## Description\n\n${description}\n\n## Inputs\n\n- 文件路径。\n- 用户问题或目标。\n- 可选输出路径，建议写入当前房间 \`artifacts/docs/\` 或当前 Agent \`workspace/\`。\n\n## Steps\n\n1. 确认文件在当前房间可读目录中，例如 \`shared/\`、\`artifacts/\` 或项目文件。\n2. 运行 \`scripts/${scriptName} <file> [output.md]\` 抽取文本/表格为 Markdown。\n3. 文件很长时按页、sheet 或标题分块阅读。\n4. 根据用户问题总结、定位或生成结构化产物。\n5. 正式产物写入房间 \`artifacts/docs/\`、\`shared/\` 或通过 FreeChat CLI 写入项目文件。\n\n## Outputs\n\n- Markdown 抽取结果。\n- 摘要、页码/sheet/段落引用。\n- 可选产物文件。\n`, 'utf8')
