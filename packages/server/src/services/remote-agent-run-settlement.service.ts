...
export function completeRemoteRun(auth: ConnectorAuth, runId: string, payload: any = {}) {
  ...
  const rawOutput = String(payload.output || payload.summary || '')
  const chatOutput = sanitizeAiCompletionForChat(rawOutput)
  const skipAsyncHooks = payload.__skipAsyncHooks === true
  const now = Date.now()
  ...
  if (!skipAsyncHooks) void conversationMemoryService.onRunCompleted({ roomId: run.room_id, agentId: run.agent_id, input: run.input, output: chatOutput || rawOutput, finishedAt: now })
    .catch((err) => console.error('[conversation-memory] run hook failed', err))
  if (!skipAsyncHooks && (run.run_source === 'agent.mentioned' || run.run_source === 'handoff') && chatOutput && payload.responseMode !== 'tool_only' && payload.responseMode !== 'silent') {
    const existingMessage = db.prepare(`
      ...
      LIMIT 1
    `).get(run.room_id, run.agent_id, chatOutput, Number(run.started_at || 0)) as any
    if (!existingMessage) void messageService.createMessage(run.room_id, run.agent_id, (db.prepare('SELECT name FROM agents WHERE id = ?').get(run.agent_id) as any)?.name || 'Agent', 'ai', chatOutput)
