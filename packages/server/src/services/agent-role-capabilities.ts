...
const assistantCapabilities: AgentRoleCapability[] = [
  ...
  },
  {
    ...
    title: 'Agent 协调',
    promptRules: [
      '遇到复合任务、长内容任务或命中其他 Agent 专长的事项，应优先查看成员并分派给合适 Agent。',
      '有匹配 Agent时禁止自己直接产出最终成品，应创建真实任务计划交互卡，或在用户已明确要求立即执行时分派 Agent任务/子任务。',
      '不要通过普通聊天 @ 另一个 Agent 来制造自动对话；多 Agent 协作优先通过任务/子任务分派。',
    ],
    workspaceRules: [
      '复杂/跨 Agent/长内容/命中其他 Agent 专长的事项：必须先用 ./freechat members list 查看协作者。',
      '有匹配 Agent时，必须创建 task plan 预览或用 --assignee 分派 Agent。',
      '不要通过普通聊天 @ 另一个 Agent制造自动对话。',
    ],
    autoPromptRules: [
      '遇到复合任务、长内容任务、或明显包含房间其他 Agent 专长的任务，必须先用 ./freechat members list 查看协作者。',
      '如果有匹配 Agent，禁止直接产出最终成品，必须通过 ./freechat task plan create-json 发真实任务计划交互卡，或在用户已明确要求立即执行时用 ./freechat task create / task subtask add --assignee 分派 Agent。',
      '不要通过普通聊天 @ 另一个 Agent制造自动对话。',
    ],
  },
  {
