...
export const api = {
  ...
  getWorkgroupEntry: (token: string, ref?: string) => request<{ entry: any }>(`/workgroup-entries/${token}${ref ? `?ref=${encodeURIComponent(ref)}` : ''}`),
  joinWorkgroupEntry: (token: string, ref?: string) => request<{ entry: any; room: any }>(`/workgroup-entries/${token}/join${ref ? `?ref=${encodeURIComponent(ref)}` : ''}`, { method: 'POST' }),
  ...
  sendRoomMessageWithFiles: (id: string, content: string, files: File[], options: { mentions?: any[]; reply_to?: string } = {}) => {
    const form = new FormData()
    form.append('content', content)
    if (options.mentions?.length) form.append('mentions', JSON.stringify(options.mentions))
    if (options.reply_to) form.append('reply_to', options.reply_to)
