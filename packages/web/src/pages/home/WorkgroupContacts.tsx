...
export function WorkgroupContacts({ workgroups, reloadWorkgroups }: { workgroups: any[]; reloadWorkgroups: () => void }) {
  ...
  const loadEntryAnalytics = async (entry: any) => { if (!selectedId) return; const data = await api.getWorkgroupEntryAnalytics(selectedId, entry.id); setAnalytics((prev) => ({ ...prev, [entry.id]: data })) }
...
function Metric({ label, value }: { label: string; value: any }) { return <div className="rounded-lg bg-white px-2 py-2"><p className="text-[10px] text-gray-400">{label}</p><p className="mt-0.5 font-semibold text-gray-800">{value}</p></div> }
function formatCredits(value: number) { return `${Number(value || 0).toFixed(4)} cr` }
