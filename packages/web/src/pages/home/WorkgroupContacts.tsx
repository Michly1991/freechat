...
function roleLabel(role?: string) { return ({ owner: '所有者', admin: '管理员', member: '成员', viewer: '访客', assistant: '接待', expert: '专家', operator: '运营' } as any)[role || ''] || role || '-' }
...
export function WorkgroupContacts({ workgroups, reloadWorkgroups }: { workgroups: any[]; reloadWorkgroups: () => void }) {
  ...
  const [agentForm, setAgentForm] = useState({ agentId: '', role: 'assistant' })
  ...
  const addAgent = async () => { if (selectedId && agentForm.agentId) { await api.addWorkgroupAgent(selectedId, agentForm); setAgentForm({ agentId: '', role: 'assistant' }); await refreshCurrent() } }
  ...
  return <div className="space-y-4">
    ...
    {list && <section className="space-y-4 rounded-2xl border border-gray-100 bg-white p-4"><div><h3 className="font-semibold text-gray-800">{list.workgroup.name}</h3><p className="mt-1 text-sm text-gray-500">{list.workgroup.description || '暂无描述'}</p></div>
      <div className="grid gap-3 lg:grid-cols-2"><Panel title="人员" icon={<Users className="h-4 w-4" />}>{(list.members || []).map((m: any) => <div key={m.user_id} className="flex items-center justify-between gap-2 rounded-xl border bg-white p-2"><span className="min-w-0 truncate text-sm">{m.nickname || m.username}</span>{canManage ? <div className="flex gap-1"><select value={m.role} onChange={(e) => updateMember(m, e.target.value)} className="rounded-lg border px-2 py-1 text-xs"><option value="owner">owner</option><option value="admin">admin</option><option value="member">member</option><option value="viewer">viewer</option></select><button onClick={() => removeMember(m)} className="text-xs text-red-500">移出</button></div> : <span className="text-xs text-gray-400">{roleLabel(m.role)}</span>}</div>)}</Panel>
      <Panel title="Agent 资源" icon={<Bot className="h-4 w-4" />}>{canManage && <div className="mb-2 grid gap-2 sm:grid-cols-[1fr_auto_auto]"><select value={agentForm.agentId} onChange={(e) => setAgentForm({ ...agentForm, agentId: e.target.value })} className="min-w-0 rounded-xl border px-2 py-2 text-sm"><option value="">选择可加入 Agent</option>{availableAgents.map((a) => <option key={a.id} value={a.id}>{a.name}</option>)}</select><select value={agentForm.role} onChange={(e) => setAgentForm({ ...agentForm, role: e.target.value })} className="rounded-xl border px-2 py-2 text-sm"><option value="assistant">assistant</option><option value="expert">expert</option><option value="operator">operator</option><option value="member">member</option></select><button onClick={addAgent} className="rounded-xl bg-blue-600 px-3 py-2 text-sm text-white">加入</button></div>}{(list.agents || []).length === 0 ? <Empty text="还没有 Agent。先加入一个 Agent，才能创建分享入口。" /> : (list.agents || []).map((a: any) => <div key={a.id} className="rounded-xl border bg-white p-3"><div className="flex items-center justify-between gap-2"><span className="min-w-0 truncate text-sm font-medium">{a.name}</span>{canManage && <button onClick={() => removeAgent(a)} className="inline-flex items-center gap-1 text-xs text-red-500"><Trash2 className="h-3.5 w-3.5" />移出</button>}</div><div className="mt-2 flex items-center gap-2">{canManage ? <select value={a.workgroup_role} onChange={(e) => updateAgent(a, e.target.value)} className="rounded-lg border px-2 py-1 text-xs"><option value="assistant">assistant</option><option value="expert">expert</option><option value="operator">operator</option><option value="member">member</option></select> : <span className="text-xs text-gray-400">{roleLabel(a.workgroup_role)}</span>}<span className="rounded bg-gray-100 px-2 py-0.5 text-xs text-gray-500">{a.status}</span></div></div>)}</Panel></div>
      <Panel title="分享入口" icon={<Link className="h-4 w-4" />} wide>{canManage && <div className="mb-4 rounded-2xl border border-emerald-100 bg-emerald-50/40 p-3"><div className="mb-3 flex items-start gap-2 text-sm text-emerald-800"><ShieldCheck className="mt-0.5 h-4 w-4 shrink-0" /><p>公开链接，登录用户都可访问；系统会为每个使用者创建独立对话，费用由使用者自己承担。</p></div><div className="grid gap-2 sm:grid-cols-2"><input value={entryForm.title} onChange={(e) => setEntryForm({ ...entryForm, title: e.target.value })} className="rounded-xl border px-3 py-2 text-sm" placeholder="入口标题" /><select value={entryForm.agentId} onChange={(e) => setEntryForm({ ...entryForm, agentId: e.target.value })} className="rounded-xl border px-3 py-2 text-sm"><option value="">选择接待 Agent</option>{(list.agents || []).map((a: any) => <option key={a.id} value={a.id}>{a.name}</option>)}</select><input value={entryForm.description} onChange={(e) => setEntryForm({ ...entryForm, description: e.target.value })} className="rounded-xl border px-3 py-2 text-sm sm:col-span-2" placeholder="入口说明" /><textarea value={entryForm.welcomeMessage} onChange={(e) => setEntryForm({ ...entryForm, welcomeMessage: e.target.value })} rows={2} className="rounded-xl border px-3 py-2 text-sm sm:col-span-2" placeholder="欢迎语（可选，会显示在新对话里）" /><input value={entryForm.maxUses} onChange={(e) => setEntryForm({ ...entryForm, maxUses: e.target.value.replace(/[^0-9]/g, '') })} className="rounded-xl border px-3 py-2 text-sm" placeholder="最大使用次数（可选）" /><input type="datetime-local" value={entryForm.expiresAt} onChange={(e) => setEntryForm({ ...entryForm, expiresAt: e.target.value })} className="rounded-xl border px-3 py-2 text-sm" /><label className="flex items-center gap-2 text-sm text-gray-600"><input type="checkbox" checked={entryForm.enabled} onChange={(e) => setEntryForm({ ...entryForm, enabled: e.target.checked })} />启用入口</label><div className="flex justify-end gap-2"><button onClick={resetEntryForm} className="rounded-xl bg-white px-3 py-2 text-sm text-gray-600">清空</button><button onClick={submitEntry} className="rounded-xl bg-emerald-600 px-3 py-2 text-sm text-white">{editingEntryId ? '保存入口' : '创建入口'}</button></div></div></div>}{entries.length === 0 ? <Empty text="还没有分享入口。创建一个入口，把指定 Agent 通过公开链接分享给别人使用。" /> : <div className="grid gap-3 md:grid-cols-2">{entries.map((e) => <EntryCard key={e.id} entry={e} onCopy={copy} onEdit={editEntry} onToggle={toggleEntry} onDelete={deleteEntry} />)}</div>}</Panel>
...
function EntryCard({ entry, onCopy, onEdit, onToggle, onDelete }: { entry: any; onCopy: (url: string) => void; onEdit: (entry: any) => void; onToggle: (entry: any) => void; onDelete: (entry: any) => void }) {
  const url = shareUrl(entry.token)
  return <div className="rounded-2xl border bg-white p-4 shadow-sm"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><div className="flex items-center gap-2"><Link className="h-4 w-4 text-blue-500" /><b className="truncate text-sm text-gray-800">{entry.title}</b><span className={`rounded-full px-2 py-0.5 text-[10px] ${entry.enabled ? 'bg-emerald-50 text-emerald-600' : 'bg-gray-100 text-gray-500'}`}>{entry.enabled ? '启用' : '停用'}</span></div><p className="mt-1 text-xs text-gray-500">接待：{entry.agentName || entry.agentId}</p>{entry.description && <p className="mt-2 line-clamp-2 text-xs text-gray-500">{entry.description}</p>}</div></div><div className="mt-3 grid gap-2 rounded-xl bg-gray-50 p-3 text-xs text-gray-500"><p>使用次数：{entry.usedCount || 0}{entry.maxUses ? ` / ${entry.maxUses}` : ' / 不限'}</p><p className="flex items-center gap-1"><CalendarClock className="h-3.5 w-3.5" />过期时间：{formatDate(entry.expiresAt)}</p><p>费用：链接使用者自付费</p></div><div className="mt-3 flex flex-wrap gap-2">{url && <button onClick={() => onCopy(url)} className="inline-flex items-center gap-1 rounded-lg bg-blue-50 px-2 py-1 text-xs text-blue-600"><Copy className="h-3.5 w-3.5" />复制</button>}{url && <a href={url} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 rounded-lg bg-gray-100 px-2 py-1 text-xs text-gray-600"><ExternalLink className="h-3.5 w-3.5" />预览</a>}<button onClick={() => onEdit(entry)} className="rounded-lg bg-gray-100 px-2 py-1 text-xs text-gray-600">编辑</button><button onClick={() => onToggle(entry)} className="rounded-lg bg-gray-100 px-2 py-1 text-xs text-gray-600">{entry.enabled ? '停用' : '启用'}</button><button onClick={() => onDelete(entry)} className="rounded-lg bg-red-50 px-2 py-1 text-xs text-red-500">删除</button></div></div>
}
