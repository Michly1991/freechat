        ...
        {agents.length === 0 ? <p className="text-sm text-gray-400">暂无 Agent，点击右上角新建一个。</p> : [...agents].sort((a, b) => (a.builtInKey === 'default_assistant' ? -1 : 0) - (b.builtInKey === 'default_assistant' ? -1 : 0)).map((a) => (
          <div key={a.id} className="p-3 rounded-xl border border-gray-100 flex items-start justify-between gap-3">
            <div className="flex items-start gap-3 min-w-0">
              ...
              <div className="min-w-0"><div className="flex items-center gap-2 flex-wrap"><p className="text-sm font-medium truncate">{a.name}</p><span className="text-[10px] px-1.5 py-0.5 rounded bg-violet-50 text-violet-600">{a.roleType === 'assistant' ? '助理' : '专家'}</span>{a.builtInKey === 'default_assistant' && <span className="text-[10px] px-1.5 py-0.5 rounded bg-amber-50 text-amber-600">默认助理</span>}</div>{a.description && <p className="text-xs text-gray-500 mt-1 line-clamp-2">{a.description}</p>}{a.specialties?.length > 0 && <p className="text-xs text-gray-400 mt-1 truncate">{a.specialties.join('、')}</p>}</div>
            </div>
