                  ...
                  {editing ? (
                    <div className="space-y-4">
                      ...
                      <div className="rounded-xl border border-gray-100 bg-gray-50 p-3 space-y-3">
                        <div className="flex items-center justify-between"><div><p className="text-sm font-medium text-gray-700">全局 Agent</p><p className="text-xs text-gray-400">选择场景包含的全局 Agent，并可直接配置模板；第一个默认作为房间助理。</p></div><button onClick={() => { const first = agents.find((item) => !form.agents.some((a) => a.agentId === item.id)); setForm({ ...form, agents: [...form.agents, { agentId: first?.id || '', name: first?.name || '', roleType: first?.roleType, autoEnabled: form.agents.length === 0, priority: form.agents.length }] }); if (first?.id) setSelectedGlobalAgentId(first.id) }} className="px-2.5 py-1.5 rounded-lg bg-blue-50 text-xs text-blue-600 hover:bg-blue-100">+ 添加</button></div>
                        ...
                        {form.agents.map((agent, index) => {
                          return <div key={index} className="bg-white border border-gray-100 rounded-xl p-3 space-y-2"><div className="grid grid-cols-1 sm:grid-cols-[1fr_110px_90px] gap-2"><select value={agent.agentId || ''} onChange={(e) => { const selected = agents.find((item) => item.id === e.target.value); setForm({ ...form, agents: form.agents.map((item, i) => i === index ? { ...item, agentId: e.target.value, name: selected?.name || item.name, roleType: selected?.roleType || item.roleType } : item) }); setSelectedGlobalAgentId(e.target.value) }} className="w-full px-2 py-2 border rounded-lg text-xs text-gray-700"><option value="">选择全局 Agent</option>{agents.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select><label className="text-xs flex items-center gap-1 h-9"><input type="checkbox" checked={!!agent.autoEnabled} onChange={(e) => setForm({ ...form, agents: form.agents.map((item, i) => i === index ? { ...item, autoEnabled: e.target.checked } : { ...item, autoEnabled: e.target.checked ? false : item.autoEnabled }) })} />房间助理</label><div className="flex items-center justify-end gap-2"><button onClick={() => setSelectedGlobalAgentId(agent.agentId)} className="text-xs text-blue-600">配置</button><button onClick={() => setForm({ ...form, agents: form.agents.filter((_, i) => i !== index) })} className="text-xs text-red-500">删除</button></div></div></div>
                        })}
                      </div>
