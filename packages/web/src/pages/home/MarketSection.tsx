...
export function MarketSection(props: MarketSectionProps) {
  const { marketKind, setMarketKind, agents, scenes, reloadScenes, reloadAgents, openCreateAgent, openEditAgent, deleteAgentFromContacts } = props
  return (
    <section className="bg-white sm:rounded-xl sm:border border-gray-200 p-4 sm:p-5 mb-4 sm:mb-6">
      ...
      {marketKind === 'agents' && <AgentContacts mode="market" agents={agents.filter((agent) => agent.marketListed)} reloadAgents={reloadAgents} openCreateAgent={openCreateAgent} openEditAgent={openEditAgent} deleteAgentFromContacts={deleteAgentFromContacts} />}
