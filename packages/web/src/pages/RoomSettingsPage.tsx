...
import { Activity, BarChart3, Bot, Settings, Shield, Users, Wrench, X } from 'lucide-react'
...
import { AgentRunsPanel } from './room/components/AgentRunsPanel'

type RoomSettingsTab = 'basic' | 'collaborators' | 'agent' | 'runs' | 'diagnostics' | 'advanced'
...
export default function RoomSettingsPage() {
  ...
  const loadAll = async () => {
    try {
      ...
      try { const a = await api.getAgents(); setAgents((a.agents || []).filter((agent: any) => agent.canUse)) } catch {}
    } catch { navigate('/') }
  }
  const saveRoom = async () => { if (!canEditRoom) { feedback.warning('你没有权限修改房间设置'); return } try { await api.updateRoom(roomId!, { name: editName, description: editDesc }); feedback.success('保存成功') } catch (e: any) { feedback.error(e.message || '保存失败') } }
  ...
  const generateInvite = async () => {
    ...
    try { const data = await api.createInvite(roomId!, { role: 'viewer' }); setInviteCode(data.code); setInviteUrl(data.url?.startsWith('http') ? data.url : `${window.location.origin}/join?code=${data.code}`) }
    catch (e: any) { feedback.error('生成失败: ' + (e.message || JSON.stringify(e))) }
  }
  const removeAgent = async (agentId: string) => { if (!canEditRoom) { feedback.warning('你没有权限移除 Agent'); return } try { await api.removeRoomAgent(roomId!, agentId); feedback.success('Agent 已移除'); loadAll() } catch (e: any) { feedback.error(e.message || '移除失败') } }
  const addAgent = async (agent: any, options?: { roomRole?: 'assistant' | 'specialist'; autoEnabled?: boolean }) => {
    ...
    try {
      ...
      loadAll()
    } catch (e: any) { feedback.error(e.message || '添加失败') }
  }
  ...
  const addUserCollaborator = async (targetUserId: string, role: 'owner' | 'editor' | 'viewer' = 'editor') => { if (!canEditRoom) { feedback.warning('你没有权限添加协作者'); return } try { await api.addRoomMember(roomId!, targetUserId, role); feedback.success('人员已添加'); setUserSearchResults([]); setCollaboratorQuery(''); loadAll() } catch (e: any) { feedback.error(e.message || '添加失败') } }
  ...
  const restartAgent = async (agent: any) => {
    ...
    if (!ok) return
    try { await api.restartRoomAgent(roomId, agent.id, true); feedback.success('Agent 已重启'); loadAll() }
    catch (e: any) { feedback.error(e.message || '重启失败') }
  }
  ...
  const renderCollaborators = () => (
    <section className="bg-white rounded-lg border border-gray-200 p-5">
      <div className="flex items-center justify-between gap-3 mb-4">
        ...
        <button disabled={!canEditRoom} onClick={() => canEditRoom ? setShowAddCollaborator(true) : feedback.warning('你没有权限添加协作者')} className={`px-3 py-2 rounded-lg text-sm shrink-0 ${canEditRoom ? 'bg-blue-600 text-white hover:bg-blue-700' : 'bg-gray-100 text-gray-400 cursor-not-allowed'}`}>添加协作者</button>
      </div>
      ...
      <div className="space-y-2">
        ...
        {roomAgents.map((a) => <div key={a.id} className="flex items-center justify-between p-3 rounded-lg border border-gray-100"><div className="min-w-0"><span className="text-sm font-medium">{a.name}</span><span className={`ml-2 text-[11px] px-2 py-0.5 rounded-full ${a.autoEnabled ? 'bg-blue-100 text-blue-700' : 'bg-gray-100 text-gray-500'}`}>{a.autoEnabled ? '协调者' : 'Agent'}</span>{a.description && <span className="text-xs text-gray-400 ml-2">{a.description}</span>}</div><button disabled={!canEditRoom} onClick={() => removeAgent(a.id)} className={`text-xs ${canEditRoom ? 'text-red-500 hover:text-red-700' : 'text-gray-300 cursor-not-allowed'}`}>移除</button></div>)}
      </div>
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-4xl mx-auto p-4 space-y-6">
        {activeTab === 'basic' && <><section className="bg-white rounded-lg border border-gray-200 p-5"><h2 className="text-lg font-semibold text-gray-800 mb-4">基本信息</h2><div className="space-y-3"><div><label className="text-sm text-gray-600 block mb-1">房间名称</label><input value={editName} onChange={(e) => setEditName(e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm" /></div><div><label className="text-sm text-gray-600 block mb-1">描述</label><textarea value={editDesc} onChange={(e) => setEditDesc(e.target.value)} rows={3} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm" /></div><button disabled={!canEditRoom} onClick={saveRoom} className={`px-4 py-2 rounded-lg text-sm ${canEditRoom ? 'bg-blue-600 text-white hover:bg-blue-700' : 'bg-gray-100 text-gray-400 cursor-not-allowed'}`}>保存</button></div></section>{roomId && <RoomAnalyticsPanel roomId={roomId} />}</>}
        {activeTab === 'collaborators' && renderCollaborators()}
        {activeTab === 'agent' && <div className="space-y-6"><section className="bg-white rounded-lg border border-gray-200 p-5"><div className="flex items-center justify-between gap-3 mb-4"><div><h2 className="text-lg font-semibold text-gray-800">Agent 配置</h2><p className="text-xs text-gray-400 mt-1">房间 Agent、协调者和梦境复盘集中在这里。</p></div><button disabled={!canEditRoom} onClick={() => { if (!canEditRoom) { feedback.warning('你没有权限添加 Agent'); return }; setActiveTab('collaborators'); setShowAddCollaborator(true); setAddKind('agents') }} className={`px-3 py-2 rounded-lg text-sm shrink-0 ${canEditRoom ? 'bg-blue-600 text-white hover:bg-blue-700' : 'bg-gray-100 text-gray-400 cursor-not-allowed'}`}>添加 Agent</button></div><div className="space-y-2">{roomAgents.length === 0 && <p className="text-sm text-gray-400">暂无 Agent</p>}{roomAgents.map((a) => <div key={a.id} className="flex items-center justify-between p-3 rounded-lg border border-gray-100"><div className="min-w-0"><span className="text-sm font-medium">{a.name}</span><span className={`ml-2 text-[11px] px-2 py-0.5 rounded-full ${a.autoEnabled ? 'bg-blue-100 text-blue-700' : 'bg-gray-100 text-gray-500'}`}>{a.autoEnabled ? '协调者' : 'Agent'}</span>{a.lastError && <p className="text-xs text-red-500 mt-1 truncate">{a.lastError}</p>}</div><button disabled={!canEditRoom} onClick={() => removeAgent(a.id)} className={`text-xs ${canEditRoom ? 'text-red-500 hover:text-red-700' : 'text-gray-300 cursor-not-allowed'}`}>移除</button></div>)}</div></section>{roomId && <AgentDreamPanel roomId={roomId} canEdit={canEditRoom} />}</div>}
        {activeTab === 'runs' && roomId && <AgentRunsPanel roomId={roomId} roomAgents={roomAgents} restartAgent={restartAgent} feedback={feedback} />}
