...
import { BarChart3, Bot, FileText, Settings, Shield, Users, Wrench } from 'lucide-react'
...
type RoomSettingsTab = 'basic' | 'collaborators' | 'pages' | 'agent' | 'diagnostics' | 'advanced'
const tabs: Array<{ id: RoomSettingsTab; label: string; icon: any }> = [
  ...
  { id: 'pages', label: '页面与文件', icon: FileText },
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-4xl mx-auto p-4 space-y-6">
        ...
        {activeTab === 'pages' && <section className="bg-white rounded-lg border border-gray-200 p-5"><h2 className="text-lg font-semibold text-gray-800 mb-2">页面与文件</h2><p className="text-sm text-gray-500 leading-6">页面、文件 Tab 和项目文件目录配置后续集中放在这里。当前文件内容仍在房间内“文件/页面”面板管理。</p><button onClick={() => navigate(`/room/${roomId}`)} className="mt-4 bg-blue-600 text-white px-4 py-2 rounded-lg text-sm hover:bg-blue-700">回房间管理页面和文件</button></section>}
