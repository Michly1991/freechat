  ...
  const renderTaskCard = (task: any, colKey?: string) => {
    ...
    return (
      <div key={task.id} className={`fc-enter fc-card-hover bg-white rounded-xl p-3 sm:p-3 shadow-sm border ${getEffectiveTaskStatus(task) === 'review' ? 'border-purple-200 fc-review-glow' : 'border-gray-100'}`}>
        ...
        {task.description && <p className="text-xs text-gray-400 mt-1 line-clamp-2 break-words [overflow-wrap:anywhere]">{task.description}</p>}
        ...
        <div className="mt-3 grid grid-cols-2 sm:flex gap-2">
          ...
          <button onClick={() => { if (window.confirm(`确定删除任务“${task.title}”？`)) deleteTask(task) }} className="fc-pressable rounded-lg border border-red-100 bg-white px-3 py-2 text-sm sm:text-xs font-medium text-red-500 hover:bg-red-50 active:bg-red-100">删除</button>
        </div>
