...
export function AgentModelDialog({ mode = 'room-override', roomId, agent, onClose, onSaved, feedback }: any & { mode?: Mode }) {
  ...
  const title = isAgentDefault ? '设置 Agent 默认模型' : '设置本房间模型覆盖'
  const subtitle = isAgentDefault ? `${agent.name} · 保存在通讯录 Agent 上，项目默认继承。` : `${agent.name} · 只覆盖当前项目；留空保存会恢复继承 Agent 默认模型。`
  ...
  return <div className="fixed inset-0 z-[70] flex items-end sm:items-center justify-center bg-black/40 p-0 sm:p-4" onClick={onClose}>
    <form onSubmit={save} className="w-full sm:max-w-lg rounded-t-3xl sm:rounded-2xl bg-white p-5 shadow-xl space-y-4" onClick={(e) => e.stopPropagation()}>
      ...
      {!isAgentDefault && <div className="rounded-xl bg-blue-50 px-3 py-2 text-xs text-blue-700">当前房间可覆盖通讯录 Agent 默认模型；如果不选择模型配置来源，则恢复继承默认。</div>}
      <label className="block"><span className="text-xs font-medium text-gray-500">模型配置来源</span><select value={form.modelProfileId} onChange={(e) => { const p = profiles.find((x) => x.id === e.target.value); setForm({ ...form, modelProfileId: e.target.value, model: p?.defaultModel || '', allowPaidSharedModel: form.allowPaidSharedModel && p?.visibility === 'shared' }) }} className="mt-1 w-full rounded-lg border border-gray-300 px-3 py-2 text-sm"><option value="">{isAgentDefault ? '不设置 / 使用平台默认' : '继承 Agent 默认模型'}</option>{profiles.map((p) => <option key={p.id} value={p.id}>{p.name} · {sourceLabel(p)} · {p.defaultModel || '未设默认'}</option>)}</select></label>
