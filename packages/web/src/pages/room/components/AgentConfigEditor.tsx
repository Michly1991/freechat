...
export function AgentConfigEditor({ agentId, feedback, scopeLabel, emptyText = '请选择 Agent', onSaved }: Props) {
  ...
  return <div className="space-y-4">
    ...
    <section className="bg-white border border-gray-200 rounded-2xl p-4 shadow-sm">
      <div className="flex items-center justify-between gap-2 mb-3"><div><h3 className="font-semibold text-gray-900">模型与运行</h3><p className="text-xs text-gray-400 mt-1">在这里设置自己 Agent 的默认模型，所有房间、私聊和工作组入口默认统一生效；房间里仅作为特殊覆盖。</p></div>{canEdit && <button onClick={() => setShowModelDialog(true)} className="text-sm text-blue-600">设置默认模型</button>}</div>
      <div className="rounded-xl bg-gray-50 px-3 py-2 text-sm text-gray-600">
        {agent.defaultModelConfig?.model || agent.defaultModelConfig?.modelProfileName ? <><p className="font-medium text-gray-800">{agent.defaultModelConfig.modelProfileName || '默认模型配置'}{agent.defaultModelConfig.model ? ` · ${agent.defaultModelConfig.model}` : ''}</p><p className="text-xs text-gray-500 mt-1">所有房间默认统一使用 · {agent.defaultModelConfig.modelSource === 'user_owned' ? '我的模型' : agent.defaultModelConfig.modelSource === 'platform' ? '平台模型' : agent.defaultModelConfig.modelSource === 'marketplace' ? '共享/市场模型' : '模型配置'}{agent.defaultModelConfig.allowPaidSharedModel ? ' · 允许按模型定价售卖' : ''}</p></> : <p>未设置 Agent 默认模型；所有房间默认使用平台默认模型，或由个别房间单独覆盖。</p>}
      </div>
    </section>
