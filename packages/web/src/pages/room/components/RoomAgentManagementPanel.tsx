...
export function RoomAgentManagementPanel({ roomAgents, feedback }: Props) {
  ...
  const addSceneAgent = () => {
    ...
    setSceneForm({ ...sceneForm, agents: [...sceneForm.agents, { agentId: first?.id || '', name: first?.name || '', roleType: first?.roleType, autoEnabled: false, priority: sceneForm.agents.length }] })
  ...
  const setSceneAgent = (index: number, agentId: string) => {
    ...
    setSceneForm({ ...sceneForm, agents: sceneForm.agents.map((x, i) => i === index ? { ...x, agentId, name: selected?.name || x.name, roleType: selected?.roleType || x.roleType, autoEnabled: selected?.roleType === 'assistant' ? x.autoEnabled : false } : x) })
          ...
          <div className="space-y-3">{(editingSceneId === 'new' ? [{ id: 'new', name: '新场景', description: '', agents: [] }, ...scenes] : scenes).map((scene) => {
            ...
            return <div key={scene.id} className="border border-gray-100 rounded-xl p-3">
              {editing ? <div className="space-y-3">
                ...
                <div className="rounded-xl bg-gray-50 border border-gray-100 p-3 space-y-2"><div className="flex justify-between"><div><p className="text-sm font-medium text-gray-700">全局 Agent</p><p className="text-xs text-gray-400">这里编辑的是全局 Agent 模板，新项目会从它克隆；已创建项目副本不会自动变化。</p></div><button onClick={addSceneAgent} className="text-xs text-blue-600">+ 添加</button></div>{sceneForm.agents.map((item, index) => { const tmpl = agentTemplates.find((a) => a.id === item.agentId); const canAuto = tmpl?.roleType === 'assistant' || item.roleType === 'assistant'; return <div key={index} className="grid grid-cols-1 sm:grid-cols-[1fr_110px_90px] gap-2 bg-white border border-gray-100 rounded-lg p-2"><select value={item.agentId || ''} onChange={(e) => setSceneAgent(index, e.target.value)} className="px-2 py-2 border rounded text-xs"><option value="">选择全局 Agent</option>{agentTemplates.map((a) => <option key={a.id} value={a.id}>{a.name}</option>)}</select><label className={`text-xs flex items-center gap-1 ${canAuto ? '' : 'text-gray-300'}`}><input type="checkbox" disabled={!canAuto} checked={!!item.autoEnabled && canAuto} onChange={(e) => setSceneForm({ ...sceneForm, agents: sceneForm.agents.map((x, i) => i === index ? { ...x, autoEnabled: e.target.checked } : { ...x, autoEnabled: e.target.checked ? false : x.autoEnabled }) })} />自动助理</label><div className="flex justify-end gap-2"><button onClick={() => setSelectedGlobalAgentId(item.agentId)} className="text-xs text-blue-600">配置</button><button onClick={() => setSceneForm({ ...sceneForm, agents: sceneForm.agents.filter((_, i) => i !== index) })} className="text-xs text-red-500">删除</button></div></div> })}</div>
