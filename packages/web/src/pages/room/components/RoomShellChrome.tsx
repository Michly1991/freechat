...
export function DiagnosticsDialog({ showDiagnostics, setShowDiagnostics, wsStatus, hasToken, roomId, clientLogs, diagnosticsText, copyDiagnostics }: any) {
  if (!showDiagnostics) return null
  return <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/30 p-3"><div className="w-full max-w-3xl max-h-[85vh] overflow-hidden rounded-xl bg-white shadow-2xl flex flex-col"><div className="flex items-center justify-between border-b border-gray-200 px-4 py-3"><div><h3 className="font-semibold text-gray-800">诊断日志</h3><p className="text-xs text-gray-500">仅保存在当前浏览器内存，不包含完整 token</p></div><button onClick={() => setShowDiagnostics(false)} className="text-gray-400 hover:text-gray-600"><X className="w-5 h-5" /></button></div><div className="grid grid-cols-2 sm:grid-cols-5 gap-2 border-b border-gray-100 p-4 text-xs"><div><span className="text-gray-400">WS</span><div className="font-mono">{wsStatus}</div></div><div><span className="text-gray-400">Token</span><div className="font-mono">{hasToken ? 'present' : 'missing'}</div></div><div><span className="text-gray-400">Room</span><div className="font-mono truncate">{roomId}</div></div><div><span className="text-gray-400">Host</span><div className="font-mono truncate">{window.location.host}</div></div><div><span className="text-gray-400">Logs</span><div className="font-mono">{clientLogs.length}</div></div></div><div className="flex gap-2 border-b border-gray-100 p-3"><button onClick={copyDiagnostics} className="inline-flex items-center gap-1 rounded bg-blue-600 px-3 py-1.5 text-xs text-white hover:bg-blue-700"><Clipboard className="w-3.5 h-3.5" />复制日志</button><button onClick={() => clearClientLogs()} className="inline-flex items-center gap-1 rounded bg-gray-100 px-3 py-1.5 text-xs text-gray-600 hover:bg-gray-200"><Trash2 className="w-3.5 h-3.5" />清空</button></div><pre className="flex-1 overflow-auto bg-gray-950 p-4 text-[11px] leading-relaxed text-gray-100 whitespace-pre-wrap">{diagnosticsText || '暂无日志'}</pre></div></div>
}
