...
import { Bell, BarChart3, KeyRound, LogOut, Shield, UserRound } from 'lucide-react'
...
type SettingsTab = 'account' | 'analytics' | 'models' | 'notifications' | 'system'
const tabs: Array<{ id: SettingsTab; label: string; icon: any }> = [
  ...
  { id: 'models', label: '我的模型', icon: KeyRound },
...
export function PersonalSettingsTabs({ onLogout }: { onLogout?: () => void }) {
  ...
  const updateSoundPref = async (next: Partial<typeof soundPrefs>) => { await unlockNotificationSound(); setSoundPrefs(setNotificationSoundPrefs(next)) }
  const testSound = async () => { await unlockNotificationSound(); await playNotificationSound('mention', 'settings-test', true) }
  const handleLogout = () => { logout(); onLogout?.() }

  return <div className="space-y-6">
    <div className="overflow-x-auto"><div className="flex gap-2 min-w-max">{tabs.map((tab) => <TabButton key={tab.id} active={activeTab === tab.id} icon={tab.icon} label={tab.label} onClick={() => setActiveTab(tab.id)} />)}</div></div>
    {activeTab === 'account' && <div className="space-y-6"><section className="bg-white rounded-xl p-5 sm:p-6 shadow-sm border border-gray-100"><h2 className="text-lg font-semibold text-gray-800 mb-4">基本信息</h2><form onSubmit={handleSaveProfile} className="space-y-4"><div className="flex items-center gap-4">{avatar ? <img src={avatar} alt="头像" className="w-16 h-16 rounded-full object-cover border border-gray-200" onError={() => setAvatar('')} /> : <div className="w-16 h-16 rounded-full bg-gradient-to-br from-blue-400 to-purple-500 flex items-center justify-center text-white text-xl font-semibold">{(nickname || user?.username || '?')[0].toUpperCase()}</div>}<div><label className="inline-flex items-center px-4 py-2 bg-blue-50 text-blue-600 rounded-lg cursor-pointer hover:bg-blue-100 text-sm">{uploadingAvatar ? '上传中...' : '上传头像'}<input type="file" accept="image/png,image/jpeg,image/webp,image/gif" className="hidden" onChange={handleAvatarUpload} disabled={uploadingAvatar} /></label><p className="text-xs text-gray-400 mt-1">支持 png / jpg / webp / gif，最大 2MB</p></div></div><div><label className="block text-sm font-medium text-gray-700 mb-1">用户名</label><input type="text" value={user?.username || ''} disabled className="w-full px-3 py-2 border border-gray-200 rounded-lg bg-gray-50 text-gray-500" /></div><div><label className="block text-sm font-medium text-gray-700 mb-1">昵称</label><input type="text" value={nickname} onChange={(e) => setNickname(e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500" /></div>{msg && <p className="text-sm text-blue-600">{msg}</p>}<button type="submit" className="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700">保存修改</button></form></section><section className="bg-white rounded-xl p-5 sm:p-6 shadow-sm border border-gray-100"><h2 className="text-lg font-semibold text-gray-800 mb-4">修改密码</h2><form onSubmit={handleChangePassword} className="space-y-4"><div><label className="block text-sm font-medium text-gray-700 mb-1">当前密码</label><input type="password" value={oldPwd} onChange={(e) => setOldPwd(e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-lg" required /></div><div><label className="block text-sm font-medium text-gray-700 mb-1">新密码</label><input type="password" value={newPwd} onChange={(e) => setNewPwd(e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-lg" required /></div>{pwdMsg && <p className="text-sm text-blue-600">{pwdMsg}</p>}<button type="submit" className="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700">更新密码</button></form></section></div>}
    {activeTab === 'analytics' && <PersonalAnalyticsPanel />}
    {activeTab === 'models' && <ModelProfilesPanel />}
