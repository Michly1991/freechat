  ...
  return <section className="bg-white rounded-xl p-5 sm:p-6 shadow-sm border border-gray-100 space-y-4">
    ...
    <div className="grid gap-4 lg:grid-cols-[260px,1fr]">
      ...
      <div className="space-y-5">
        ...
        {selected && <div className="rounded-xl border border-gray-100 p-4 space-y-3"><h3 className="font-medium text-gray-800">Credit 计费规则</h3><div className="space-y-2">{rules.length === 0 && <p className="text-sm text-gray-400">暂无规则。未配置规则的运行只记 token，不扣 credit。</p>}{rules.map((r) => <div key={r.id} className="rounded-lg bg-gray-50 px-3 py-2 text-sm text-gray-600">{r.model}：前 {fmtCredit(r.inputCreditPerMillion)}/百万，后 {fmtCredit(r.outputCreditPerMillion)}/百万，最低 {fmtCredit(r.minCreditsPerRun)}</div>)}</div><form onSubmit={saveRule} className="grid gap-2 sm:grid-cols-3"><input placeholder="模型名" value={ruleForm.model} onChange={(e) => setRuleForm({ ...ruleForm, model: e.target.value })} className="rounded-lg border border-gray-300 px-3 py-2 text-sm" /><input type="number" step="0.0001" placeholder="前 token/百万" value={ruleForm.inputCreditPerMillion} onChange={(e) => setRuleForm({ ...ruleForm, inputCreditPerMillion: Number(e.target.value) })} className="rounded-lg border border-gray-300 px-3 py-2 text-sm" /><input type="number" step="0.0001" placeholder="后 token/百万" value={ruleForm.outputCreditPerMillion} onChange={(e) => setRuleForm({ ...ruleForm, outputCreditPerMillion: Number(e.target.value) })} className="rounded-lg border border-gray-300 px-3 py-2 text-sm" /><input type="number" step="0.0001" placeholder="cache 写/百万" value={ruleForm.cacheWriteCreditPerMillion} onChange={(e) => setRuleForm({ ...ruleForm, cacheWriteCreditPerMillion: Number(e.target.value) })} className="rounded-lg border border-gray-300 px-3 py-2 text-sm" /><input type="number" step="0.0001" placeholder="cache 读/百万" value={ruleForm.cacheReadCreditPerMillion} onChange={(e) => setRuleForm({ ...ruleForm, cacheReadCreditPerMillion: Number(e.target.value) })} className="rounded-lg border border-gray-300 px-3 py-2 text-sm" /><input type="number" step="0.0001" placeholder="最低/次" value={ruleForm.minCreditsPerRun} onChange={(e) => setRuleForm({ ...ruleForm, minCreditsPerRun: Number(e.target.value) })} className="rounded-lg border border-gray-300 px-3 py-2 text-sm" /><button className="sm:col-span-3 rounded-lg bg-gray-900 px-4 py-2 text-sm text-white">保存规则</button></form></div>}
