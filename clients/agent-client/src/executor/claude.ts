...
export function writeRunContext(cfg: ClientConfig, agent: AgentCredential, event: RemoteEvent, cwd: string, spec: RuntimeSpec) {
  ...
  const knowledgeDir = join(agent.workdir || join(workRoot(), event.agentId), 'knowledge')
  mkdirSync(knowledgeDir, { recursive: true })
  writeFileSync(join(freechatDir, 'runtime-spec.json'), JSON.stringify({ version: spec.version, checksum: spec.checksum, updatedAt: spec.updatedAt, knowledgeDir }, null, 2), 'utf8')
  writeFileSync(join(freechatDir, 'KNOWLEDGE.md'), `# Agent 本地知识库\n\n本 Agent 的客户端本地知识库目录：\n\n${knowledgeDir}\n\n如任务需要背景资料，请优先按需读取该目录中的 Markdown / 文本文件。不要把知识库文件复制到聊天，除非用户明确要求。\n`, 'utf8')
