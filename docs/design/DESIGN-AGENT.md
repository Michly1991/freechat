...
Agent 被服务端触发后，服务端会先代表 Agent 发一条轻量回执消息：

```text
收到，处理中…
```

回执由服务端在启动模型前写入消息表并广播，消息 `kind = 'agent_receipt'`，`payload.status = 'accepted'`，用于告诉用户 Agent 已经收到请求，避免模型启动或执行较慢时用户误以为丢包。

适用场景：

- 人类消息触发当前房间 `auto_enabled` Agent。
- 人类 @ 一个或多个 Agent。
- 人类创建/分派任务给 Agent。

回执消息仍由对应 Agent 作为 actor 发送，但前端以更淡的状态气泡展示。自动助理构建上下文时会过滤 `agent_receipt`，避免“收到，处理中…”污染后续判断。AI 普通消息不触发其他 Agent，因此回执也不会造成 Agent 循环。
