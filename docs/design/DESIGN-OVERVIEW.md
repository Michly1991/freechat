...
- 房间设置
- 删除项目仅允许 `owner` 操作；后端必须强制校验，非 owner 返回 HTTP 403 和明确错误文案 `你没有权限删除该项目`。项目删除采用逻辑删除：`rooms.deleted_at/deleted_by` 标记后从普通项目列表隐藏，但保留 `rooms.id/name`，确保账单、运行记录和历史流水仍能关联原始房间。
- 前端危险操作区和首页项目会话的删除入口必须根据权限给出可见反馈：无权限点击时提示 `你没有权限删除该项目`，不能出现点击无效；有权限时展示二次确认后再调用删除接口。
...
## 2026-06-11 场景模板与 Agent 模板补充

- 新增场景模板概念：创建项目时可选择场景，场景把全局 Agent 模板克隆进用户自己的项目房间。
- Agent/场景模板是所有用户共享的全局配置：所有用户可见可用，但只有 owner/admin 可修改。
- 管理项目/项目房间是独立的：消息、任务、文件、页面、项目 Agent 副本和 Agent 运行记录按 room 隔离，不同用户基于同一场景看到不同对话记录。
- Agent 维护对象区分为 Agent 模板、项目内 Agent 副本和 runtime Agent 实例；模板/副本能力页不展示运行日志。
- 系统默认“助理”是内置 Agent：通讯录置顶，标记为 `default_assistant`，可查看/使用但不可编辑或删除。
- 内置“Agent 管理”场景：不再单独初始化 Agent管家、专属管理助理或管理专家；由通用默认“助理”负责入口协调和 Agent / Skill / Script / 场景管理协助；场景不再初始化页面模板。
- Agent 助理代创建/修改 Agent 或场景时与对话发起方/确认人同权，不能用 Agent 自身绕过 owner/admin 权限。
- “标签”产品语义统一改为“页面”；底层 API/表可继续兼容 `tab` 命名。
- 详细设计见 [DESIGN-SCENE-TEMPLATES.md](./DESIGN-SCENE-TEMPLATES.md)、[DESIGN-AGENT-TEMPLATES.md](./DESIGN-AGENT-TEMPLATES.md) 与 [DESIGN-LONG-TASKS.md](./DESIGN-LONG-TASKS.md)。
...
| `DESIGN-FILE-PROCESSING-BOUNDARY.md` | Server/Agent Client 两机部署下的文件上传下载与客户端本地处理边界 |
...
## 2026-06-11 通讯录新增交互统一

通讯录的 `人员 / Agent / 场景` 三个页面使用统一新增交互：

- 每个页面顶部使用同一类 header：标题、说明、主操作按钮。
- 主操作分别为：`新增人员`、`新增 Agent`、`新增场景`。
- 点击主操作后展开对应的新增/创建面板，而不是不同页面使用不同入口位置。
- 人员新增面板承载用户搜索与好友申请；Agent 新增面板承载全局 Agent 模板创建；场景新增面板承载全局场景模板创建。
- 移动端保持纵向堆叠，按钮独占一行，避免拥挤。

## UI 交互优化约定（2026-06-16）

移动端与长内容展示遵循以下约定：

- 所有聊天、交互卡、任务卡里的用户/Agent 长文本都必须支持 `break-words` 与 `overflow-wrap:anywhere`，避免 URL、路径、错误码、工具日志撑破容器。
- 任务卡移动端主操作和次要操作应自动换行/网格排列；删除类危险操作必须二次确认。
- 文件面板移动端编辑状态要明确显示“未保存”，长路径用 `title` 保留完整信息，删除文件必须二次确认。
- 个人设置页的全局分析在手机端默认折叠详情，只保留总览和展开按钮，避免设置页首屏信息过载；桌面端默认展开。

## 聊天长消息与复制（2026-06-16）

聊天面板对普通消息提供以下体验约定：

- 普通文本消息超过 1200 字或 12 行时默认折叠，用户可点击“展开全文 / 收起”。
