...
房间内会产生 token 的 Agent 命令默认只接受项目创建人触发。协作者可以参与讨论和查看共享内容，但不能直接 @Agent 或触发自动助理执行；如果协作者 @Agent，系统提示“只有项目创建人可以指挥 Agent；运行费用由项目创建人承担”。成员栏需显示“付费人 / 可指挥 Agent”。
...
- Agent 知识库由 Agent Client 本地维护；服务端弹窗第一版只展示客户端上报状态/元数据占位，不在服务端保存知识库正文。
...
- 计费责任归当前入口房间创建者，也就是点击链接并开始对话的用户；分享者/工作组 owner 不为外部使用者付费。
- Agent 启动前会做余额预检；余额不足时不启动 Agent，并提示当前用户充值。
- 分享入口页面必须明确提示“费用由你自己承担”。

## Workgroup share interaction refinement

- 工作组分享入口管理从简单列表升级为卡片式管理：状态、接待 Agent、使用次数、过期时间、费用规则、复制/预览/编辑/启停/删除操作都在卡片中展示。
- 创建/编辑入口表单增加 `maxUses` 和 `expiresAt`，并保留启用开关；公开链接说明明确写在表单上方。
- 分享落地页改为独立入口页：展示工作组、接待 Agent、Agent 描述、欢迎语、费用自付提示和公开链接登录说明；主按钮文案改为“开始我的对话”。
- 用户通过入口进入后，系统会在新房间内插入系统提示，说明当前会话来自哪个入口、使用哪个 Agent、费用由当前使用者承担。
...
- `room.handoff` 工具允许目标 Agent 来自当前工作组：如果目标律师尚未加入当前入口房间，服务端会先把该工作组 Agent 加入房间，再执行 handoff 和唤醒。
...
账单系统区分“扣费”和“用量审计”。当一次 Agent 运行有 token 用量但按规则不产生费用时（例如 Agent Client 本地模型、免费规则、付款人与 Agent owner 相同），后端仍应写入一条 `billing_ledger_entries`：
