...
每个房间创建时，系统自动创建一个服务端助理 Agent，并自动加入房间。该助理是房间默认协调者和决策者，用于参与讨论、总结信息、协调专家 Agent、创建或跟进任务。

初始化必须事务化完成：

1. 创建 `rooms` 记录
2. 创建房主 `room_members` 记录
3. 创建默认 `assistant` Agent
4. 写入 `room_agents` 绑定关系

如果任一步失败，应整体回滚。

### 6.2 Agent 在线状态

Agent 在房间内不是隐藏资源，而是房间成员视图的一部分。前端成员面板和 `@` 提及弹窗都必须显示 Agent，并显示当前状态。
...
硬删除包括数据库关联数据和房间文件目录，删除后不可恢复。系统必须在 UI 上明确提示“永久删除，不可恢复”。

### 6.5 邀请码加入项目

房间通过邀请码邀请其他用户加入。首页需要提供加入项目入口，用户输入邀请码后调用加入接口，成功后进入目标房间。
